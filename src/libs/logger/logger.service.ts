import { CONFIG_OPTIONS, PROD } from '../../common/constants/common.constant';
...
export class LoggerService implements ILoggerService {
  constructor(@Inject(CONFIG_OPTIONS) private readonly options: ILoggerModuleOptions) {}
  ...
  logger(): winston.Logger {
    ...
    const logFormat = printf(({ level, message, label, timestamp }) => {
      const removedColorText = message.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
    ...
    const logger: winston.Logger = winston.createLogger({
      ...
      transports: [
        ...
        new winstonDaily({
          ...
          filename: this.options.nodeEnv !== PROD ? `%DATE%.log` : undefined, // 파일 이름
        ...
        new winstonDaily({
          ...
          filename: this.options.nodeEnv !== PROD ? `%DATE%.error.log` : undefined, // 에러 로그는 2020-05-28.error.log 형식으로 저장
      ...
      exceptionHandlers: [
        new winstonDaily({
          ...
          datePattern: 'YYYY-MM-DD',
          dirname: logDir,
          filename: this.options.nodeEnv !== PROD ? `%DATE%.exception.log` : undefined,
          maxFiles: this.options.nodeEnv !== PROD ? 30 : 0,
          zippedArchive: true,
        }),
      ],
   ...
   *  @param {string | undefined} custom 사용자 커스텀 메시지
   *  @param {string | undefined} message 에러 메시지
   *  @param {string | undefined} name  에러 이름
   *  @param {string | undefined} stack  에러 스택
