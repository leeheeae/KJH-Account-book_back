  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if ('authorization' in req.headers && 'refresh_token' in req.headers && 'email' in req.headers) {
      ...
      try {
        ...
        const { data: user } = await this.userService.findByEmail({ email: email as string });

        try {
          let isCheckedEmailToken = false;

          if (typeof user === 'object' && _.has(user, 'refreshToken')) {
            isCheckedEmailToken = await bcrypt.compare(user.refreshToken, refreshToken as string);
        ...
        if (typeof refreshTokenDecoded === 'object' && _.has(refreshTokenDecoded, 'exp')) {
          if (typeof user === 'object' && _.has(user, 'id')) {
      ...
      try {
        const { data: user, success } = await this.userService.findById({ userId: decoded['id'] });
        // 만약에 이미 refreshToken 이 있는것을 social 로그인 할때 저장을 안해주면 여기서 에러 발생
        let isCheckedIdToken = false;

        if (typeof user === 'object' && _.has(user, 'refreshToken')) {
          isCheckedIdToken = await bcrypt.compare(user.refreshToken, refreshToken as string);
        }

        if (!success || !isCheckedIdToken) {
          // ! [ERROR] token 만료
          this.log.logger().error(this.log.loggerInfo(`ForbiddenException | 권한이 없습니다.`));
          throw new ForbiddenException();
        }
